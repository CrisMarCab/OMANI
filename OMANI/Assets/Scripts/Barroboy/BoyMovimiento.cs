...
{
    if (other.tag == "ChangeToLevel2")
    {
            Initiate.Fade("S2_Playground",Color.white,6.0f);
    }

    if (other.gameObject.tag == "StillCamera")
    {
        Debug.Log("StillCamera");

        if (FindObjectOfType<ControlCamara>().CameraWatchingPlayer == true)
        {
            FindObjectOfType<ControlCamara>().CameraWatchingPlayer = false;

        }
        else
        {
            FindObjectOfType<ControlCamara>().CameraWatchingPlayer = true;
        }
    }
}
